...
class LogitsMetadata:
    ...
    # for blend mode
    blend_seq_lens: Optional[torch.Tensor] = None
    ...
    def from_forward_batch(cls, forward_batch: ForwardBatch):
        ...
        return cls(
            ...
            blend_style=forward_batch.blend_info.blend_style
            if forward_batch.blend_info is not None
            else None,
            blend_seq_lens=forward_batch.blend_info.blend_top_lens
            if forward_batch.blend_info is not None
            else None,
        )
...
class LogitsProcessor(nn.Module):
    ...
    def forward(
        ...
        if (
            logits_metadata.is_prefill_only
            and logits_metadata.blend_style == BlendStyle.QCOMPUTE
        ...
        if (
            ...
            if logits_metadata.blend_style in (
                BlendStyle.DO_BLEND,
                BlendStyle.DO_BLEND_FINISH,
            ):
                last_index = torch.cumsum(logits_metadata.blend_seq_lens, dim=0) - 1
            else:
