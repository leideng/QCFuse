...
class TritonAttnBackend(AttentionBackend):
    ...
    def forward_extend_blend(
        ...
        """
        Forward extend with blend support for Triton backend.
        ...
        is_do_blend = (
            blend_info.blend_style == BlendStyle.DO_BLEND
            or blend_info.blend_style == BlendStyle.DO_BLEND_FINISH
        )

        # Early exit: fallback to regular extend for QCOMPUTE or DO_BLEND without top_lens
        if blend_info.blend_style == BlendStyle.QCOMPUTE:
            if blend_info.is_contextblend:
                # ContextBlend QCOMPUTE: q/k/v already processed by CacheBlender.blend()
                # q = query-only, k/v = [context + query] concatenated
                # Use ragged_positions_attention_fwd, no KV cache write
                ...
                self.ragged_positions_attention_fwd(
                    q.view(-1, layer.tp_q_head_num, layer.qk_head_dim),
                    k.view(-1, layer.tp_k_head_num, layer.qk_head_dim),
                    v.view(-1, layer.tp_v_head_num, layer.v_head_dim),
                    o.view(-1, layer.tp_q_head_num, layer.v_head_dim),
                    blend_info._ctx_q_start_loc,
                    blend_info._ctx_q_lens_t,
                    blend_info._ctx_kv_start_loc,
                    blend_info._ctx_kv_lens_t,
                    blend_info._context_q_positions,
                    blend_info._context_q_len,
                    is_causal=True,
                    sm_scale=layer.scaling,
        ...
        if is_do_blend and blend_info.blend_top_lens is None:
            return self.forward_extend(
                q, k, v, layer, forward_batch, save_kv_cache=True
            )

        # Initialize metadata on first layer, reuse on subsequent layers
        if blend_info.init_attmeta:
            self._init_triton_blend_metadata(blend_info, q.device, is_do_blend)

        # Retrieve cached params
        q_start_loc = blend_info._triton_blend_q_start_loc
        q_lens = blend_info._triton_blend_q_lens
        kv_start_loc = blend_info._triton_blend_kv_start_loc
        kv_lens = blend_info._triton_blend_kv_lens
        max_q_len = blend_info._triton_blend_max_q_len
        ...
        # window_offsets = (
        #     torch.clamp(kv_lens - sliding_window, min=0) if sliding_window > 0 else None
        # )

        # Reshape tensors for attention kernel
        ...
        # Call appropriate attention kernel based on blend mode
        if is_do_blend:
            # Get Q positions for position-based causal masking
            q_positions = blend_info._triton_blend_q_positions
    ...
    def _init_triton_blend_metadata(self, blend_info, device, is_do_blend):
        """Initialize Triton blend attention metadata on first layer."""
        chunk_bounds = blend_info.chunk_loc_list.to(device=device, dtype=torch.int32)

        if is_do_blend:
            top_lens = blend_info.blend_top_lens.to(device=device, dtype=torch.int32)
            req_len_list = blend_info.req_len_list.to(device=device, dtype=torch.long)
            num_reqs = top_lens.shape[0]

            # Q params: lengths and cumsum start locations
            q_lens = top_lens
            q_start_loc = (
                torch.nn.functional.pad(
                    torch.cumsum(top_lens[:-1], dim=0), (1, 0), value=0
                )
                if num_reqs > 1
                else torch.zeros(num_reqs, dtype=torch.int32, device=device)
            )

            # KV params: calculate from chunk bounds using request boundaries
            req_start_idx = torch.nn.functional.pad(req_len_list[:-1], (1, 0), value=0)
            kv_start_loc = chunk_bounds[req_start_idx]
            kv_lens = (chunk_bounds[req_len_list] - kv_start_loc).to(torch.int32)
            max_q_len = int(top_lens.max().item()) if top_lens.numel() > 0 else 0
            q_positions = blend_info.blend_top_indices.to(
                device=device, dtype=torch.int32
            )
        else:
            # KVCOMPUTE mode: Q and KV have identical structure
            seq_lens = (chunk_bounds[1:] - chunk_bounds[:-1]).to(torch.int32)
            q_lens = kv_lens = seq_lens
            q_start_loc = kv_start_loc = chunk_bounds[:-1].to(torch.int32)
            max_q_len = int(seq_lens.max().item()) if len(seq_lens) > 0 else 1
            q_positions = None

        # Cache metadata in blend_info for reuse across layers
        blend_info._triton_blend_q_start_loc = q_start_loc
        blend_info._triton_blend_q_lens = q_lens
        blend_info._triton_blend_kv_start_loc = kv_start_loc
        blend_info._triton_blend_kv_lens = kv_lens
        blend_info._triton_blend_max_q_len = max_q_len
        blend_info._triton_blend_q_positions = q_positions
        blend_info.init_attmeta = False
