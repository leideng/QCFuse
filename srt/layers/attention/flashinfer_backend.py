...
class FlashInferAttnBackend(AttentionBackend):
    ...
    def forward_extend_blend(
        ...
        if blend_info.blend_style is not None and blend_info.init_attmeta:
            chunk_bounds = blend_info.chunk_loc_list.to(dtype=torch.int32)
            if blend_info.blend_style == BlendStyle.DO_BLEND:
                req_len_list = blend_info.req_len_list.to(dtype=torch.long)
                top_lens = blend_info.blend_top_lens
                qo_indptr = torch.nn.functional.pad(
                    torch.cumsum(top_lens, dim=0, dtype=torch.int32), 
                    (1, 0), value=0
                )
                kv_indptr = torch.nn.functional.pad(chunk_bounds[req_len_list], (1, 0), value=0)
            else:
                qo_indptr = chunk_bounds
                kv_indptr = chunk_bounds
