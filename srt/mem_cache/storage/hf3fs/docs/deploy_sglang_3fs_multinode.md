...
## HF3fs configures
```bash
vim /path/to/sglang/benchmark/hf3fs/hf3fs_config.json
{
    "file_path_prefix": "/path/to/hicache",
...
## node1
```bash
export SGLANG_HICACHE_HF3FS_CONFIG_PATH=/path/to/sglang/benchmark/hf3fs/hf3fs_config.json
...
nohup python3 benchmark/hicache/bench_multiturn.py \
    --model-path /path/to/models/Qwen3-32B \
    --dataset-path /path/to/data/ShareGPT_V3_unfiltered_cleaned_split.json \
    --port 30000 \
