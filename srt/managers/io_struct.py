...
class GenerateReqInput(BaseReq):
    ...
    # blend additional parameters can be added here
    blend_style: Optional[str] = None
    method: Optional[str] = None
    separator: Optional[str] = None
    start: Optional[int] = 0
    ratio: Optional[float] = 0.3
    blend_sep_token: Optional[List[int]] = None
    ...
    def __getitem__(self, i):
        return GenerateReqInput(
            ...
            start=self.start,
            ratio=self.ratio,
            blend_sep_token=self.blend_sep_token,
            attn_start=self.attn_start,
            attn_end=self.attn_end,
            is_contextblend=self.is_contextblend,
            context_cache_source=self.context_cache_source,
            context_n_sink=self.context_n_sink,
            digest_ratio=self.digest_ratio,
            digest_index_method=self.digest_index_method,
            critical_layers=self.critical_layers,
            # SSD pipeline parameters
            ssd_cache_path_chunk=self.ssd_cache_path_chunk,
            ssd_cache_path_query=self.ssd_cache_path_query,
        )
